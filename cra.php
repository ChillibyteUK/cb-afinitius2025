<?php

require_once $_SERVER['DOCUMENT_ROOT'] . '/wp-load.php';

$data   = $_REQUEST['data'];
$scores = $_REQUEST['scores'];

$test = $_REQUEST['emailaddress'];
if ( '' !== $test ) {
    $results = 'http' . ( isset( $_SERVER['HTTPS'] ) ? 's' : '' ) . "://$_SERVER[HTTP_HOST]/";
    header( "Location: $results" );
    exit();
}

// SANITIZE INPUT!!!!.

$data   = json_decode( stripslashes( $data ), true );
$scores = json_decode( stripslashes( $scores ), true );


if ( ! isset( $data['contactEmail'] ) || empty( $data['contactEmail'] ) ) {
    $home = 'http' . ( isset( $_SERVER['HTTPS'] ) ? 's' : '') . "://$_SERVER[HTTP_HOST]/";
    header( "Location: $home" );
    exit();
}

// Extract company and email from the POST data.
$company = '';
$email   = '';

if ( $data['contactEmail'] ) {
    $email = $data['contactEmail'];
}
if ( $data['orgName'] ) {
    $company = $data['orgName'];
}

$post_title = trim( $company . ' | ' . $email );
$post_slug = random_str( 32 );

$post_data = array(
    'post_title'  => $post_title,
    'post_name'   => $post_slug,
    'post_type'   => 'cra',
    'post_status' => 'publish',
);
$cra_post_id = wp_insert_post( $post_data );

$data_field   = 'field_64955d11c677b';
$scores_field = 'field_649564f0c6785';

update_field( $data_field, $data, $cra_post_id );
update_field( $scores_field, $scores, $cra_post_id );

$results = 'http' . ( isset( $_SERVER['HTTPS'] ) ? 's' : '' ) . "://$_SERVER[HTTP_HOST]/cra/$post_slug/";

$to      = $data['contactEmail'];
$subject = 'Afiniti Change Accelerator Results';
$message = <<<EOT
<p>Dear {$data['contactName']},</p>
<p>Thank you for completing the online Afiniti Change Accelerator. You can view and share your results at any time here: <a href="{$results}">{$results}</a>.</p>
<p>If you would like to speak with our team about your change program, just reply to this email or complete our <a href="http://www.afiniticonsultants.com/contact-us/">online inquiry form</a> and we will get back to you.</p>
<p>Best regards,</p>
<p>Afiniti</p>
<img src="data:image/png;base64," width="200" height="70">
EOT;

$headers = array(
    'Content-Type: text/html; charset=UTF-8',
    'From: Afiniti <no-reply@afiniticonsultants.com>',
    'Reply-To: enquiries@afiniti.co.uk',
    'Bcc: enquiries@afiniti.co.uk',
);

wp_mail( $to, $subject, $message, $headers );

// redirect to results page.
header( "Location: $results" );
exit();
